...
final class MywpDeveloper {
  ...
  public static function get_filter_to_func( $filter_name = fasle ) {
    ...
    foreach( $wp_filters[ $filter_name ] as $priority => $filters ) {
      ...
      foreach( $filters as $filter_name => $filter_array ) {
        ...
        if( is_array( $filter_array['function'] ) ) {
          ...
          if( is_object( $filter_array['function'][0] ) ) {
            ...
            $print_format = sprintf( 'class %s -> %s()' , $filter_function['class'] , $filter_function['function'] );
